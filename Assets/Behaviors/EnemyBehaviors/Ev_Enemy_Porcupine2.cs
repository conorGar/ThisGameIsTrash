...
public class Ev_Enemy_Porcupine2 : MonoBehaviour
{
	...
	IEnumerator FireQuills(){
		...
		quill2.transform.rotation = Quaternion.Euler(0,0,0);

		quill2.GetComponent<Projectile>().myShadow.transform.localPosition = new Vector2(0,-.89f);
		...
		quill3.transform.rotation = Quaternion.Euler(0,0,-90);

		quill3.GetComponent<Projectile>().myShadow.transform.localPosition = new Vector2(1.66f,.27f);
		...
		quill4.transform.rotation = Quaternion.Euler(0,0,90);

		quill4.GetComponent<Projectile>().myShadow.transform.localPosition = new Vector2(-.52f,0f);
