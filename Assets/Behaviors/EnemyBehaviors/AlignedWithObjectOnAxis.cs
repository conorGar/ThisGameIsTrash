    ...
    public bool IsAligned()
    {
        switch (alignedAxis) {
            case ALIGNED_AXIS.X_AXIS:
                if (Mathf.Abs(gameObject.transform.position.x - alignedObject.transform.position.x) < threshhold)
                    return true;
                break;
            case ALIGNED_AXIS.Y_AXIS:
                if (Mathf.Abs(gameObject.transform.position.y - alignedObject.transform.position.y) < threshhold)
                    return true;
                break;
        }
