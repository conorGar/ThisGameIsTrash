...
public class GUIEffectsEditor : Editor
{
	...
	public override void OnInspectorGUI()
	{
		...
		if( effectScript.isCommonParams ){
			...
			effectScript.commonEaseType 						= (GUITween.EaseType)EditorGUILayout.EnumPopup("\tEase Type (Common)",effectScript.commonEaseType);
			if( effectScript.isRandomEaseType ){
				EditorGUILayout.HelpBox("Random ease type will be applied at the end, because you have selected random ease type in the Random Effect section below.!", MessageType.Info , false );
			}
			effectScript.commonLoopType 						= (GUITween.LoopType)EditorGUILayout.EnumPopup("\tLoop Type (Common)",effectScript.commonLoopType);
		}
		...
		if( effectScript.isRandom ){
			effectScript.isRandomEaseType 					= EditorGUILayout.Toggle( "\t\tEaseType", effectScript.isRandomEaseType );
		...
		if( !effectScript.isRandom ){
			...
			if( !effectScript.isPanel ){
				effectScript.isColorChangeFoldOpen 				= EditorGUILayout.Foldout( effectScript.isColorChangeFoldOpen, "Color" );
				if( effectScript.isColorChangeFoldOpen ){
					...
					if( effectScript.isFade ){
						if( !effectScript.isCommonParams ){
							...
							effectScript.fadeLoopType 						= (GUITween.LoopType)EditorGUILayout.EnumPopup("\t\tLoop Type", effectScript.fadeLoopType);
						}
			...
			effectScript.isPositionChangeFoldOpen 			= EditorGUILayout.Foldout( effectScript.isPositionChangeFoldOpen, "Position" );
			...
			// move from bottom
				if( !effectScript.isCommonParams ){
					...
					effectScript.isRandomPositionEaseType 		= EditorGUILayout.Toggle("\t\tRandom EaseType", effectScript.isRandomPositionEaseType);
					if( !effectScript.isRandomPositionEaseType ){
						effectScript.positionEaseType 						= (GUITween.EaseType)EditorGUILayout.EnumPopup("\t\tEase Type",effectScript.positionEaseType);
					}
					effectScript.positionLoopType 						= (GUITween.LoopType)EditorGUILayout.EnumPopup("\t\tLoop Type",effectScript.positionLoopType);
				}
			...
			effectScript.isScaleChangeFoldOpen 					= EditorGUILayout.Foldout( effectScript.isScaleChangeFoldOpen , "Scale" );
			if( effectScript.isScaleChangeFoldOpen ){
				...
				if( !effectScript.isCommonParams ){
					...
					effectScript.isRandomScaleEaseType 		= EditorGUILayout.Toggle("\t\tRandom EaseType",effectScript.isRandomScaleEaseType);
					if( !effectScript.isRandomScaleEaseType ){
						effectScript.scaleEaseType 						= (GUITween.EaseType)EditorGUILayout.EnumPopup("\t\tEase Type",effectScript.scaleEaseType);
					}
					effectScript.scaleLoopType 						= (GUITween.LoopType)EditorGUILayout.EnumPopup("\t\tLoop Type",effectScript.scaleLoopType);
				}
			...
			effectScript.isRotationChangeFoldOpen 			= EditorGUILayout.Foldout( effectScript.isRotationChangeFoldOpen, "Rotation");
			...
			// rotation
				...
				if( effectScript.isRotation ){
					...
					if( !effectScript.isCommonParams ){
						...
						effectScript.isRandomRotationEaseType 		= EditorGUILayout.Toggle("\t\tRandom EaseType",effectScript.isRandomRotationEaseType);
						if( !effectScript.isRandomRotationEaseType ){
							effectScript.rotationEaseType 						= (GUITween.EaseType)EditorGUILayout.EnumPopup("\t\tEase Type",effectScript.rotationEaseType);
						}
						effectScript.rotationLoopType 						= (GUITween.LoopType)EditorGUILayout.EnumPopup("\t\tLoop Type",effectScript.rotationLoopType);
					}
					...
					if( effectScript.rotationLoopType == GUITween.LoopType.none ){
